...
class APIRequest:
    ...
    def __get(self):
        r = requests.get(self.context.get_url(), params=self.context.get_parameters(),
        ...
        return APIResponse(r.status_code, headers, body)

    def __post(self):
        r = requests.post(self.context.get_url(), headers=self.context.get_headers(),
...
class APIContext(dict):
    ...
    def __init__(self, api_key='', public_key='', ssl=False, method_type=APIMethodType.GET, address='', port=80,
